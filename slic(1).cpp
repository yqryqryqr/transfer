...
int main(){
    ...
    for(;;) {
        ...
        std::vector<std::vector<double>> flux(nCells + 2 * ghost_cells - 1, std::vector<double>(3));
...
//      dt = update(grid, dx, Gamma, CFL);
    ...
    std::ofstream outFile("C:\\Users\\huawei\\Desktop\\xingfm\\SLIC_results.dat");
    for (int i = ghost_cells; i < nCells + ghost_cells; i++) {
        pvar pri = con2pri(grid[i].rho, grid[i].rhov, grid[i].E, Gamma);
